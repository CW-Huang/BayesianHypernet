...
def get_lbda(tau, length_scale, drop_prob=None):
    ...
    lbda = length_scale**2 / 2 / tau # we don't divide by the 2 * N(=dataset size) as in Gal, since our prior handles this scaling
...
if __name__ == '__main__':
    ...
    if grid_search:
        # TODO: better grid...
        length_scales = [.0001]#length_scales = [.1, .01,  .001] # length scale should be smaller!
        taus = [1000.]#taus = [.01, .1, 1, 10., 100.] # tau should be larger!
        lr0s = [.001]#lr0s = [.01, .001, .0001]
        drop_probs = [.1, .05, .02, .01, .005, .002, .001]
