...
def run_episode(env,
    ...
    variance_steps = np.zeros(max_step)
    ...
    for i in range(max_step):
        ...
        """
        Choose action that has the highest variance over the Q network
        """
        #actions based on maximising the q value
        action, variance = agent.dropout_highest_variance(state)
        variance_steps[i] = variance
        ...
        if len(memory.memory) > batch_size:  # DQN Experience Replay
            ...
            '''
            Dropout probability should be 1 here
            '''
    ...
    return loss_v, w1_m, w2_m, w3_m, total_reward, step_length, variance_steps
...
variance_episodes = np.zeros((max_n_steps, max_n_ep))
...
for e in range(Experiments):
    ...
    while avg_Rwd < min_avg_Rwd and ep < max_n_ep:
        ...
        """
        loss_v, w1_m, w2_m, w3_m, cum_R, step_length, variance_steps = run_episode(env, agent, None, memory, batch_size=batch_size, discount=discount,
                                                      max_step=max_n_steps)
        
        variance_episodes[:, ep] = variance_steps
    ...
    variance_steps_episodes = variance_episodes


    np.save('/Users/Riashat/Documents/PhD_Research/BASIC_ALGORITHMS/My_Implementations/Exploration_DQN/All_Results/'  + 'Cum_Rwd_' + 'Dropout_Highest_Variance_' + str(e) + '.npy', total_reward)
    np.save('/Users/Riashat/Documents/PhD_Research/BASIC_ALGORITHMS/My_Implementations/Exploration_DQN/All_Results/'  + 'Variance_Values_' + 'Dropout_Highest_Variance_' + str(e) + '.npy', variance_steps_episodes)
...
np.save('/Users/Riashat/Documents/PhD_Research/BASIC_ALGORITHMS/My_Implementations/Exploration_DQN/All_Results/'   + 'Average_Cum_Rwd_' + 'Dropout_Highest_Variance_' + '.npy', Average_Cum_Rwd)


print "All Experiments DONE - Deep Q Learning"


print "Dropout with Highest Variance for Exploration DONE"
