...
from get_proper_regression_data import get_regression_dataset
...
if __name__ == '__main__':
    ...
    parser.add_argument('--length_scale',default=1e-6, type=float)
    parser.add_argument('--tau',default=1e3, type=float)
    ...
    for trial in range(n_splits):
        ...
        print ("Experiment Trial :", trial)

        input_dim, train_x, train_y, test_x , test_y = get_regression_dataset(dataset, trial)
        if model == 'MCDropout':
            t0 = time.time()
        ...
        path = save_dir
        name = '{}/boston_regression_m{}p{}c{}lr0{}seed{}reinit{}flow{}trial{}tau{}l{}'.format(
            path,
            model,
            drop_prob,
            coupling,
